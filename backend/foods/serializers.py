...
class PublishFoodSerializer(serializers.ListSerializer):
    def to_representation(self, data):
        data = data.filter(is_publish=True)
        return super(PublishFoodSerializer, self).to_representation(data)
...
class FoodSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Food
        list_serializer_class = PublishFoodSerializer
